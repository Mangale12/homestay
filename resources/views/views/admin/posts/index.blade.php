                            ...
                            <form action="" method="GET" id="author_filter" class="ml-1 sorting_bar">
                                <select name="author" id="author" onchange="author_filter()">
                                    ...
                                    @foreach ($authors as $user)
                                        <option value="{{$user->id}}" @isset($author) @if ($author == $user->id) selected @endif @endisset>{{$user->name}}</option>
                                    @endforeach
                    ...
                    <form action="{{route('delete_all')}}" method="POST" id="del">
                        ...
                        <table class="table table-striped projects" id="myTable">
                            ...
                            <tbody>
                                ...
                                @else
                                @php
                                    $all_posts=$posts->where('author',Auth::user()->id)
                                @endphp
                                @endhasanyrole
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="{{ route('post.view_pdf',$post->id) }}" target="_blank">{{ $post->pdf_file }}</a></td>
                                        <td>
                                            @if (!empty($cat))
                                                @php
                                        ...
                                        <td>
                                            <input type="checkbox" data-id="{{ $post->id }}" name="trending" class="js-trending" {{ $post->trending == '1' ? 'checked' : '' }}>

                                        </td>
                                        <td>
                                            <input type="checkbox" data-id="{{ $post->id }}" name="banner_news" class="js-banner_news" id="banner" {{ $post->banner_news == '1' ? 'checked' : '' }}>
                                        ...
                                        <td>
                                            ...
                                            @endphp
                                                ...
                                                <span class="badge badge-pill badge-success">Published</span>
                                                @php
                                                App\Models\Post::where('id',$post->id)->update(['status' => 'published']);
                                                @endphp
                                                @else
                                        ...
                                        <td>
                      					@if (App\Models\User::where('id',$post->author)->first()!=null)
                                                {{App\Models\User::where('id',$post->author)->first()->name}}
                                            @endif
                      					</td>
                                        <td>
